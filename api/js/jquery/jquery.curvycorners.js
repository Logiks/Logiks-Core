...
function curvyObject() {
    ...
    this.bgImage 			= (bgImage != "none" && bgImage!="initial") ? bgImage : "";
    this.boxContent 		= $(this.box).html();
	
	// Style/fix
	$(this.box).css("padding","0px");
    ...
    this.applyCorners = function() {
        ...
        for(var t = 0;t < 2;t++) {
            switch(t) {
                ...
                if(this.settings.tl || this.settings.tr) {
					...
					$(this.topContainer).css({ width:"100%", "font-size":"1px", overflow:"hidden", position:"absolute", "padding-left":this.borderWidth, "padding-right":this.borderWidth, height:topMaxRadius + "px",top:0 - topMaxRadius + "px",left:0 - this.borderWidth + "px"});
                };
                break;
                case 1:
                if(this.settings.bl || this.settings.br) {
					...
					$(this.bottomContainer).css({ width:"100%", "font-size":"1px", overflow:"hidden", position:"absolute", "padding-left":this.borderWidth, "padding-right":this.borderWidth, height:botMaxRadius,bottom:0 - botMaxRadius + "px",left:0 - this.borderWidth + "px"});
                };
                break;
            };
        };
		...
		if(this.settings.autoPad == true) {
			...
			// Float added to remove gaps with negative margins!
            $(contentContainer).css({position:"relative", float:"left", "margin-top":"-" + topMaxRadius + "px", "margin-bottom":"-" + botMaxRadius + "px","padding-bottom":this.boxPaddingBottom,"padding-top":this.boxPaddingTop,"padding-left":this.boxPaddingLeft,"padding-right":this.boxPaddingRight}).html(this.boxContent).addClass = "autoPadDiv";
			$(clearDiv).css("clear","both");
            this.contentDIV = this.box.appendChild(contentContainer);
			this.contentDIV = this.box.appendChild(clearDiv);
        };
    ...
    this.drawPixel = function(intx, inty, colour, transAmount, height, newCorner, image, cornerRadius) {
        ...
        if(image == -1 && this.bgImage != "") {
			...
			//"backgroundposition":"-" + ((this.boxWidth - cornerRadius - this.borderWidth)+ intx) + "px -" + (($(this.box).height() + topMaxRadius  - this.borderWidth )-inty) + "px"
        }
        else
        {
